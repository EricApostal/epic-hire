...
typedef _CacheKey = ({String identifier, Identified key});
...
/// A simple cache for [IdentifiedEntity]s.
class Cache<T> with MapMixin<Identified, T> {
  static final Expando<Map<_CacheKey, _CacheEntry>> _stores = Expando(
    'Cache store',
  );
  ...
  void filterItems() {
    final keys = List.of(
      _store.keys.where((element) => element.identifier == identifier),
    );

    if (config.maxSize != null && keys.length > config.maxSize!) {
      keys.sort(
        (a, b) => _store[a]!.accessCount.compareTo(_store[b]!.accessCount),
      );
  ...
  @override
  void operator []=(Identified key, T value) {
    assert(
      value is! ManagedIdentifiedEntity || value.id == key,
  ...
  @override
  Iterable<Identified> get keys => _store.keys
  ...
  static Map<String, Map<Identified, Object?>> cachesFor(Wrapper client) {
    final store = _stores[client];
    if (store == null) {
      return {};
    }

    final result = <String, Map<Identified, Object?>>{};
