...
T? maybeParse<T, U>(dynamic object, T Function(U) parse) {
  ...
  if (object is! U) {
    throw FormatException(
      'Unexpected type (expected $U, got ${object.runtimeType})',
      object,
    );
  }
...
List<T> parseMany<T, U>(List<dynamic> objects, [T Function(U)? parse]) {
  ...
  return UnmodifiableListView(
    List.generate(objects.length, growable: false, (index) {
      final raw = objects[index];

      if (raw is! U) {
        throw FormatException(
          'Unexpected type (expected $U, got ${raw.runtimeType})',
          raw,
        );
      }
...
List<T>? maybeParseMany<T, U>(dynamic object, [T Function(U)? parse]) =>
    maybeParse<List<T>, List<dynamic>>(
      object,
      (object) => parseMany(object, parse),
    );
