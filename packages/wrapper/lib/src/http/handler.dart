...
import 'package:wrapper/src/http/bucket.dart';
import 'package:wrapper/src/http/response.dart';
import 'package:wrapper/src/utils/iterable_extension.dart';
...
class HttpHandler {
  ...
  ///
  /// If no requests have been completed, this getter returns [Duration.zero].
  Duration get realLatency =>
      _realLatencies.isEmpty
          ? Duration.zero
  ...
  ///
  /// This method calls [WrapperPlugin.interceptRequest] on all plugins registered to the [client] which may intercept the [request].
  ...
  Future<HttpResponse> _execute(HttpRequest request) async {
    ...
    do {
      ...
      final globalWaitTime =
          (request.applyGlobalRateLimit
    ...
    try {
      ...
      final prepared = request.prepare(client);
      final response = await httpClient.send(prepared);
  ...
  Future<HttpResponse> _handle(
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      parsedResponse = await HttpResponseSuccess.fromResponse(
        request,
        response,
      );
    } else {
    ...
    if (parsedResponse.statusCode == 429) {
      try {
        ...
        final retryAfter = Duration(
          milliseconds: ((responseBody["retry_after"] as double) * 1000).ceil(),
        );
